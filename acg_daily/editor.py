...
最终分享给群友的标题、摘要和分类应使用自然简洁的简体中文。日语、英语等外语作品名，如果知道中国大陆常用译名就改用译名；不确定时只可调用一次“批量译名核对”工具，并在同一次调用中列出全部需要核对的作品名。工具优先返回 Bangumi 的多个中文词条候选，请结合新闻上下文自行判断同名作品；仅无中文候选的标题才会网页搜索兜底。联网只用于核对译名，不能补充新闻事实。Fate、VTuber 等国内仍普遍保留原名的作品可以保留原写法。

无论是否调用过工具，最后一条回复都必须只输出一个完整 JSON 对象，不得输出 Markdown 代码块、分析、译名列表、工具调用说明或任何 JSON 外文字。JSON schema：
...
items 是始终必填的数组：即使没有新闻可选，也必须返回 "items":[]；不得省略、改名或返回对象。article_id 必须来自输入候选资讯。不得输出 URL、来源名或发布日期。输入未提供排行榜时可以省略 ranking_items；提供排行榜时 ranking_items 也必须存在，并完整翻译全部指定名次的作品名，rank 不得重复、增删或调整顺序。"""
...
def build_editor_retry_prompt(prompt: str, lookup_result: str) -> str:
    ...
    return (
        ...
        + lookup_result[:6000]
        + "\n\n上一轮未返回合规日报 JSON。现在不要再次调用工具，且只能输出完整 JSON 对象：必须包含 items 数组（无新闻也写 []）；提供排行榜时还必须包含完整 ranking_items 数组。不得输出解释、译名列表或 Markdown。"
    )
