...
def normalized_lookup_titles(value: object, max_title_chars: int = 120) -> list[str]:
    ...
    for raw_title in value:
        if not isinstance(raw_title, str):
            continue
        title = " ".join(raw_title.split())[:max_title_chars]
...
def parse_bangumi_candidates(payload: object) -> list[BangumiCandidate]:
    ...
    for subject in data:
        ...
        raw_name_cn = subject.get("name_cn")
        if not isinstance(raw_name, str) or not isinstance(raw_name_cn, str):
            continue
        name = " ".join(raw_name.split())[:160]
        name_cn = " ".join(raw_name_cn.split())[:160]
        subject_type = subject.get("type")
        if not name or not name_cn or type(subject_type) is not int:
            continue
        ...
        candidate = BangumiCandidate(
            ...
            date=" ".join(subject.get("date", "").split())[:20]
            if isinstance(subject.get("date"), str)
            else "",
        )
...
def lookup_references_for_text(
    ...
    """Relate an original title to the Bangumi candidates it contained."""
